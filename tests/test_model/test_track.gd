...
func test_initialization():
	...
	var track = Track.new(60, 4, 3, 5, Instruments.Instrument.ACOUSTIC_BASS, note)
	assert_eq(track.getRate(), 60)
	assert_eq(track.getMeterTop(), 4)
	assert_eq(track.getMeterBottom(), 3)
	assert_eq(track.getOctave(), 5)
	assert_eq(track.getInstrument(), Instruments.Instrument.ACOUSTIC_BASS)
...
func test_set_notes():
	...
	var track = Track.new(60, 4, 4, 5, Instruments.Instrument.ACOUSTIC_GRAND_PIANO, [])
...
func test_set_rate():
	var track = Track.new(0, 4, 4, 5, Instruments.Instrument.ACCORDION, [])
...
func test_set_instrument():
	var track = Track.new(0, 4, 4, 5, Instruments.Instrument.ACCORDION, [])
	track.setInstrument(Instruments.Instrument.CELESTA)
	assert_eq(track.getInstrument(), Instruments.Instrument.CELESTA)
...
func test_to_string():
	var track = Track.new(10, 4, 4, 5, Instruments.Instrument.ACCORDION, [])
